...
<h4 align = "center">图1.2</h4>
如图1.2所示，对第 $l$ 层进行 Dropout，保留某个神经元的概率为 keep_prob 。前向传播的时候让某个神经元的激活值以一定概率 1 - keep_prob 停止工作（即把该神经元的激活值设为0），为了不影响整个网络输出的期望值，未被Dropout的神经元的激活值除以 keep_prob，具体见算法1.1：
```
Train:
    d[l] = np.random.rand(a[l].shape[0],a[l].shape[1]) < keep_prob
    a[l] = np.multiple(a[l],d[l])
    a[l] /= keep_prob
